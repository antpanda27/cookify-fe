...
// to fix the TS errors for categories properties, check interfaces
  return (
    <div className="my-15">
      ...
      <div className="flex w-full justify-between mt-4 flex-wrap">
        {categories.slice(0, 6).map((category) => (
          <Card
            key={category.id}
            className={`border-0 bg-gradient-to-t from-${category.color} to-white to-75%`}
          >
            <CardContent className="grid place-items-center justify-center aspect-[0.9] xl:w-33 max-xl:w-20 max-sm:w-20 max-sm:p-1">
              <Image
                ...
                alt={category.name}
                className="drop-shadow-md aspect-square max-w-25 max-xl:w-18 max-lg:w-10"
                width={100}
                height={100}
              />
              <div className="justify-self-center max-2xl:text-base max-lg:text-xs font-semibold pt-6">
                {category.name}
              </div>
            </CardContent>
          </Card>
